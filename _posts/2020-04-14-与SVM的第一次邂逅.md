...
f(x)=w^Tx+b
$$
这里的$\vec w$和$\vec x$均为多维向量，其中$\vec x$表示特征(features)，如人的高矮胖瘦等特征就由不同的$x_i$来表示，若干个特征构成了“人”这一个集合体，同理某张图片，某段一维信号也均有若干个特征所表示，这些特征共同构成了高维空间，我们所要做的就是通过调控一组权重$\vec w$，来寻找某个Hyperplane，将具有不同特性的数据集分开，$b$偏移量(bias)，是起阈值调控作用的标量，对output的数值起到影响。
